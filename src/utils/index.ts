 ...
 */
export function getByteLen(val: string | any[]) {
  let len = 0
...
// 验证是否为正整数
export function formatter(val: string) {
  if (val.length === 1) {
...
// toast 详情跳转拦截权限
export function permissionDetail(this: any, url = '') {
  const permissionList = this.$store.state.user.user.stringPermissions
  const permission = permissionList.some((item: string) => item === url)
  if (!permission) {
    this.$fail('暂无详情权限！')
    return false
  } else {
...
// 判断跳转页面参数是否为null
export function checkRouteIsNull(this: any, val: null) {
  if (val === null) {
...
export function HandlePages(page: { total: null | undefined }) {
